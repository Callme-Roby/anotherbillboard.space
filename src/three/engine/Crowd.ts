...
/** How far the crowd drifts toward either edge of the viewport, in world units. */
const WALK_RANGE = 1.5;
...
export class Crowd {
  ...
  /**
   * Walks everyone toward the pointer. `pointer` is -1 at the left edge
   * of the viewport, +1 at the right.
   */
  update(delta: number, pointer: number): void {
    ...
    for (const member of this.members) {
      ...
      const target = member.homeX + pointer * WALK_RANGE * member.reach;
